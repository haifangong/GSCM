...
from model import MMGraphCat, MMGraphCatAtt, MMGraph # no qkv and residual, no residual, qkv+residual
...
def eval_model(test_loader, model):
    ...
    with torch.no_grad():
        for data in test_loader:
            ...
            out = model((data_graph_1, data_graph_2))
            preds.append(out)  # Detach and send predictions to CPU
            ys.append(gt)
    ...
    for i, (train_index, valid_index) in enumerate(five_fold_index):
        ...
        for epoch in range(1, args.epochs + 1):
            for data in train_loader:
                ...
                out = model((data_graph_1, data_graph_2)).squeeze(1)

                loss = loss_function(out, torch.tensor(np.asarray(gt)).cuda().float())
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model', type=str, default='mma',
                        help='mma, mmac, mmc')
